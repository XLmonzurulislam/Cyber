...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/orders", isAuthenticated, isVerified, async (req, res, next) => {
    try {
      ...
      // Send notification to admin users (in a real app, we would have a way to identify admins)
      websocketService.broadcast({
        type: 'notification',
        payload: {
          title: 'New Order',
          message: `A new order for ${order.serviceName} has been placed`,
          orderId: order.id,
          timestamp: Date.now()
        }
      });
  ...
  app.patch("/api/admin/orders/:id/status", isAdmin, async (req, res, next) => {
    try {
      ...
      websocketService.sendOrderUpdate(orderId, updatedOrder.userId, status);
